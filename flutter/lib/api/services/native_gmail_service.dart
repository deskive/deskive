...
/// Service for handling native Google Sign-In for Gmail integration
/// This bypasses the web OAuth flow and uses native Google Sign-In instead
class NativeGmailService {
  static final NativeGmailService _instance =
      NativeGmailService._internal();
  static NativeGmailService get instance => _instance;

  NativeGmailService._internal();

  factory NativeGmailService() => _instance;
  ...
  /// Connect to Gmail using native Google Sign-In
  /// Returns the connection details if successful
  Future<EmailConnection> connectWithNativeSignIn() async {
    ...
    try {
      ...
      final response = await _apiClient.post(
        '/workspaces/$workspaceId/email/connect-native',
        data: {
      ...
      if (data is Map<String, dynamic>) {
        return EmailConnection.fromJson(data);
      }
      ...
      throw Exception(
        e.response?.data?['message'] ??
            'Failed to connect Gmail with native sign-in',
      );
    } catch (e) {

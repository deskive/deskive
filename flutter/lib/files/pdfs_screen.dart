...
class _PdfsScreenState extends State<PdfsScreen> {
  ...
  List<file_model.File> _pdfFiles = [];
  ...
  Future<void> _fetchPdfFiles() async {
    ...
    try {
      // Use the by-type API endpoint to fetch PDFs
      final pdfFiles = await _fileService.getFilesByType(category: 'pdfs');
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchQuery.isNotEmpty ? 'No PDFs found' : 'No PDF documents',
  ...
  List<file_model.File> _getFilteredFiles() {
    var filtered = _pdfFiles.where((file) {
  ...
  void _showPropertiesDialog(String fileName, String fileId) {
    final file = _pdfFiles.firstWhere((f) => f.id == fileId);
    final fileSize = _formatFileSize(int.tryParse(file.size) ?? 0);
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            _buildPropertyRow('Type', 'PDF Document'),

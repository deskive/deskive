...
class SpreadsheetsScreen extends StatefulWidget {
  const SpreadsheetsScreen({super.key});

  @override
  State<SpreadsheetsScreen> createState() => _SpreadsheetsScreenState();
}

class _SpreadsheetsScreenState extends State<SpreadsheetsScreen> {
  ...
  List<file_model.File> _spreadsheetFiles = [];
  ...
  Future<void> _initializeAndFetchFiles() async {
    ...
    await _fetchSpreadsheetFiles();
  }

  Future<void> _fetchSpreadsheetFiles() async {
    setState(() {
      _isLoading = true;
    });

    try {
      // Use the by-type API endpoint to fetch spreadsheets
      final spreadsheetFiles = await _fileService.getFilesByType(category: 'spreadsheets');

      setState(() {
        _spreadsheetFiles = spreadsheetFiles;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              ...
              const PopupMenuItem(value: 'Excel', child: Text('Excel Spreadsheets')),
              const PopupMenuItem(value: 'Sheets', child: Text('Google Sheets')),
              const PopupMenuItem(value: 'CSV', child: Text('CSV Files')),
      ...
      body: RefreshIndicator(
        onRefresh: _fetchSpreadsheetFiles,
        child: _buildContent(),
      ),
    );
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchQuery.isNotEmpty ? 'No spreadsheets found' : 'No spreadsheets',
            ...
            Text(
              'Upload spreadsheets to get started',
  ...
  List<file_model.File> _getFilteredFiles() {
    var filtered = _spreadsheetFiles.where((file) {
  ...
  String _getFileType(String mimeType) {
    ...
    if (mimeType.contains('sheets') || mimeType.contains('google')) return 'Sheets';
    if (mimeType.contains('csv') || mimeType == 'text/csv') return 'CSV';
    return 'Other';
  }

  IconData _getFileIcon(String mimeType) {
    if (mimeType.contains('excel') || mimeType.contains('spreadsheet')) return Icons.table_chart;
    if (mimeType.contains('sheets') || mimeType.contains('google')) return Icons.grid_on;
    if (mimeType.contains('csv') || mimeType == 'text/csv') return Icons.grid_4x4;
    return Icons.insert_drive_file;
  }

  Color _getFileColor(String mimeType) {
    if (mimeType.contains('excel') || mimeType.contains('spreadsheet')) return Colors.green;
    if (mimeType.contains('sheets') || mimeType.contains('google')) return Colors.teal;
    if (mimeType.contains('csv') || mimeType == 'text/csv') return Colors.blue;
    return Colors.grey;
  }
  ...
  void _showPropertiesDialog(String fileName, String fileId) {
    final file = _spreadsheetFiles.firstWhere((f) => f.id == fileId);
    final fileSize = _formatFileSize(int.tryParse(file.size) ?? 0);
    final fileType = _getFileType(file.mimeType);

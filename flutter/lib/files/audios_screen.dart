...
class _AudiosScreenState extends State<AudiosScreen> {
  ...
  List<file_model.File> _audioFiles = [];
  ...
  Future<void> _fetchAudioFiles() async {
    ...
    try {
      // Use the by-type API endpoint to fetch audio files
      final audioFiles = await _fileService.getFilesByType(category: 'audio');
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              ...
              const PopupMenuItem(value: 'MP3', child: Text('MP3 Audio')),
              const PopupMenuItem(value: 'WAV', child: Text('WAV Audio')),
              const PopupMenuItem(value: 'FLAC', child: Text('FLAC Audio')),
              const PopupMenuItem(value: 'AAC', child: Text('AAC Audio')),
              const PopupMenuItem(value: 'OGG', child: Text('OGG Audio')),
            ],
          ),
          PopupMenuButton<String>(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchQuery.isNotEmpty ? 'No audio files found' : 'No audio files',
  ...
  List<file_model.File> _getFilteredFiles() {
    var filtered = _audioFiles.where((file) {
  ...
  Color _getFileColor(String mimeType) {
    if (mimeType.contains('mp3') || mimeType.contains('mpeg')) return Colors.purple;
    if (mimeType.contains('wav')) return Colors.blue;
    if (mimeType.contains('flac')) return Colors.indigo;
    if (mimeType.contains('aac')) return Colors.deepPurple;
    if (mimeType.contains('ogg')) return Colors.blueGrey;
    return Colors.grey;
  }
  ...
  void _showPropertiesDialog(String fileName, String fileId) {
    final file = _audioFiles.firstWhere((f) => f.id == fileId);
    final fileSize = _formatFileSize(int.tryParse(file.size) ?? 0);
    final fileType = _getFileType(file.mimeType);
